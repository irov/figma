...
- The public header defines `FIGMA_SDK_VERSION` as `7`; hosts pass it to `figma_runtime_create`. Version mismatch returns `FIGMA_RESULT_VERSION_MISMATCH` before reading the runtime descriptor or invoking allocator hooks, and the output handle remains `NULL`. Increment `FIGMA_SDK_VERSION` for every incompatible public ABI change.
